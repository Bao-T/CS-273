...
public:
	...
	void update(int clock) 
	{
		...
		// the gates are open - ready to serve!
		if (the_queue.size() < gate) {

			// move a plane from the landing queue to the service queue
			for (int i = the_queue.size(); i < gate; i++) //Will put planes into service until the limit of gates is reached
			{
