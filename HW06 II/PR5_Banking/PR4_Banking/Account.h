...
public:
	...
	std::string to_string() {
		...
		ss << "Phone #: " << this->customer->get_address() << std::endl;
		ss << "Customer ID: " << this->customer->get_customer_number() << std::endl;
		ss << "Checking Interest -- " << this->customer->get_checking_interest() << "%        Check Charge-- $" << this->customer->get_check_charge() << "        Overdraft Penalty-- $" << this->customer->get_overdraft_penalty() << std::endl;
		ss << "  Balance: " << balance << std::endl;
		ss << "  Account ID: " << account_number << std::endl;
		return ss.str();
	}
	void add_interest()
	{
		double amt = balance*(this->get_customer()->get_checking_interest());
		balance += amt;
		std::string fees = get_fees();
		Transaction *tran = NULL;
		tran = new Transaction(this->customer->get_customer_number(), "Add interest", amt, fees);
...
public:
	...
	std::string to_string() {
		...
		ss << "Phone #: " << this->customer->get_address() << std::endl;
		ss << "Customer ID: " << this->customer->get_customer_number() << std::endl;
		ss << "Saving Interest -- " << this->customer->get_saving_interest() << "%        Check Charge-- $" << this->customer->get_check_charge() << "        Overdraft Penalty-- $" << this->customer->get_overdraft_penalty() << std::endl;
		ss << "  Balance: " << balance << std::endl;
		ss << "  Account ID: " << account_number << std::endl;
		return ss.str();
	}
	void add_interest()
	{
		double amt = balance*(this->get_customer()->get_saving_interest());
		balance += amt;
		std::string fees = get_fees();
		Transaction *tran = NULL;
		tran = new Transaction(this->customer->get_customer_number(), "Add interest", amt, fees);
